# OTTM<>게임사 - 멤버십 (커스텀 로그인 방식)

### 개요

1. OTTM 은 immutable 이라는 third party 로그인을 지원
    1. immutable 은 google, apple, email 로그인 지원
2. immutable 을 통해 OTTM 에 로그인 하면 회원은 자동으로 지갑 주소가 생성되어 있음
...
1. 커스텀 인증 키 발급
    1. [https://developers.withhive.com/ko/api/hive-server-api/auth/authv4-authenticatecustom/](https://developers.withhive.com/ko/api/hive-server-api/auth/authv4-authenticatecustom/)
2. 토큰 유효성 검증
    1. [https://developers.withhive.com/ko/api/hive-server-api/auth/authv4-verifytoken/](https://developers.withhive.com/ko/api/hive-server-api/auth/authv4-verifytoken/)